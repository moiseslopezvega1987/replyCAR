...
/**
 *
 * Esta clase se encarga de generar la factura de vehículos en PDF para su
 * exportación
 *
 * @author Moisés López Vega
 */
public class GenerarPDFFacturaVehiculo {
    
    /**
     * Constructor de la clase GenerarPDFFacturaVehiculo. Inicializa la conexión con la base
     * de datos.
     */
    public GenerarPDFFacturaVehiculo() {
    }

    /**
     * Método para generar la factura de vehículo en PDF para su exportación
     * @param veh vehículo a facturar
     * @param cli cliente
     */
    public void generarFactura(vehiculo veh, cliente cli) {

        try {
            String rutaArchivo = "C:/facturas/" + veh.getNfra_venta() + ".pdf";
            ...
            Paragraph titulo = new Paragraph("FACTURA VENTA - VEHÍCULO", fontTitulo);
            ...
            tablaFactura.addCell(veh.getNfra_venta());

            tablaFactura.addCell("Fecha Factura:");
            tablaFactura.addCell(veh.getFecha_venta().format(formatter));

            tablaFactura.addCell("Nº Cliente:");
            tablaFactura.addCell(String.valueOf(veh.getId_cliente()));
            ...
            // ================= DATOS VEHÍCULO =================
            Paragraph vehTitulo = new Paragraph("Datos del Vehículo", new Font(Font.FontFamily.HELVETICA, 14, Font.BOLD));
            ...
            tablaVehiculo.setWidths(new float[]{1, 2});

            tablaVehiculo.addCell("Modelo:");
            tablaVehiculo.addCell(veh.getModelo());
            tablaVehiculo.addCell("Matrícula:");
            tablaVehiculo.addCell(veh.getMatricula());
            tablaVehiculo.addCell("Bastidor:");
            tablaVehiculo.addCell(veh.getBastidor());
            tablaVehiculo.addCell("Color:");
            tablaVehiculo.addCell(veh.getColor());
            tablaVehiculo.addCell("Extras:");
            tablaVehiculo.addCell(veh.getExtras());
            tablaVehiculo.addCell("Comentarios:");
            tablaVehiculo.addCell(veh.getComentarios());
            ...
            tablaImportes.addCell(formato.format(veh.getPrecio_venta()));

            tablaImportes.addCell("IVA (21 %):");
            tablaImportes.addCell(formato.format(veh.getIva_venta()));

            tablaImportes.addCell("Importe Total:");
            tablaImportes.addCell(formato.format(veh.getTotal_venta()));
